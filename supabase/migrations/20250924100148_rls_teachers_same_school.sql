-- Ensure a minimal profiles table that maps auth users -> school & role
...
-- Drop any permissive policies from earlier smoke-tests
...
-- Shared predicate: current user is a teacher at same school
-- We inline this in each USING / WITH CHECK for clarity.

-- SCHOOLS: teachers can read ONLY their own school row
...
-- STUDENTS: teachers can select/insert/update/delete ONLY within their school
...
-- CLASSES: teachers can select/insert/update/delete ONLY within their school
...
-- PROFILES table: user can select own profile (helps client fetch current role/school)
...
-- (optionally allow teachers to update only their own profile; comment out if not desired)
